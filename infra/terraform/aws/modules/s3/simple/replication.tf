resource "aws_s3_bucket_replication_configuration" "this" {
  for_each = { for k, v in local.buckets : k => v if v.replication_configuration != null }
  bucket   = each.key
  ...
  dynamic "rule" {
    for_each = flatten(try([each.value.replication_configuration["rule"]], [each.value.replication_configuration["rules"]], []))
